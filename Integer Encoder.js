...
globalThis.enc=(a,b,$='0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_!#$%&()*+,-./:;<=>?@[]^{|}~',e='0123456789abcde',f=BigInt,h='slice',i=(a,b,c=k+$,d=1n,e=b?-d--:'',g=f(c[j]),h=g-d)=>{if(e)while(a[d])e=++e*g+f(c.indexOf(a[d++]));else if(h||![e=c.repeat([++a])])do e=c[(b=(g-d*g+h*a)/(h*d))%g]+e;while(d*=g,g<=b);return e},j='length',[k]=[$,$]=$.split(/(?=[^]$)/))=>b?new TextDecoder().decode(Uint8Array.from({length:(a=i(i(([b,a]=[a[h](0,b=a.indexOf($)),a[h](b+1)])[b=i(i(a[h](0,a=(a[j]-[i(b,1,k)])/2),1),0,e),1][h](a),1),0,e+'f'))[j]},(d,f)=>''+i((b[f-h]||'')+a[f],1,e+'f'),h=a[j]-b[j])):i(f((a=i(i(new TextEncoder().encode(a).forEach((c,d)=>{b+=(c=2-(c=i(f(c),0,e+'f'))[j]&&d?c+0:(c[1]||'')+c[0])[0];a+=c[1]||''},a=b='')||a,1,e)),b=i(i(b,1,e+'f')))[j]-a[j]),0,k)+$+a+b;
