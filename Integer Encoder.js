...
globalThis.enc=(a,b,$='0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ_!#$%&()*+,-./:;<=>?@[]^{|}~',e='0123456789abcde',f=BigInt,g=e+'f',h='slice',i=(a,b,c=k+$,d=1n,e=b?-d--:l,g=f(c[j]),h=g-d)=>{if(e)while(a[d])e=++e*g+f(c.indexOf(a[d++]));else if(h||![e=c.repeat([++a])])do e=c[(b=(g-d*g+h*a)/(h*d))%g]+e;while(d*=g,g<=b);return e},j='length',[k]=[$,$]=$.split(/(?=[^]$)/),l='')=>b?new TextDecoder().decode(Uint8Array.from({length:(a=i(i(([b,a]=[a[h](0,b=a.indexOf($)),a[h](b+1)])[b=i(i(a[h](0,a=(a[j]-[b&&i(b,1,k)+1n])/2),1),0,e),1][h](a),1),0,g))[j]},(d,f)=>l+i((b[f-h]||l)+a[f],1,g),h=a[j]-b[j])):((h=(a=i(i(new TextEncoder().encode(a).forEach((d,h)=>{b+=(d=2-(d=i(f(d),0,g))[j]&&h?d+0:(d[1]||l)+d[0])[0];a+=d[1]||l},a=b=l)||a,1,e)),b=i(i(b,1,g)))[j]-a[j])?i(f(h-1),0,k):l)+$+a+b;
